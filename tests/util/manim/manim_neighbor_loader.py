...
class ManimNeighborLoader(Scene):
    def construct(self):
        ...
        def create_nodes(neighbor_ids : List[AnyStr]) -> Dict[int, VGroup]:
            ...
            center_dot = Dot(point=ORIGIN, radius=0.2, color=YELLOW)
            center_label = Text(str(center_node), font_size=28).next_to(center_dot, DOWN)
        ...
        def animate_hops(node_objs: Dict[int, VGroup]) -> None:
            ...
            for nid in hop_1:
                self.play(FadeIn(node_objs[nid]), run_time=0.3)
            self.wait(0.5)
