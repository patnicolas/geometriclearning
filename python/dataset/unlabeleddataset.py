...
class UnlabeledDataset(TDataset):
    ...
    def from_numpy(cls, data: np.array, transform: Optional[Callable] = None) -> Self:
        ...
        @rtype: TensorDataset
        """
        return cls(torch.from_numpy(data), transform)

    @classmethod
    def from_list(cls, data: List[List[float]], transform: Optional[Callable] = None) -> Self:
        """
        ...
        return cls(torch.Tensor(data), transform)

    @classmethod
    def from_df(cls, df: pd.DataFrame, transform: Optional[Callable] = None) -> Self:
        """
    ...
    @classmethod
    def from_file(cls, filename: AnyStr, columns: List[AnyStr], transform: Optional[Callable] = None) -> Self:
        """
        ...
        try:
            ...
            return UnlabeledDataset.from_df(df, transform)
