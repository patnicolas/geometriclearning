...
class SimplicialVisualization(object):
    ...
    def __init__(self, simplicial_feature_set: SimplicialElements, attributes: Dict[AnyStr, Any]):
    ...
    def __draw_faces(self, node_pos: np.array, face_label_pos: np.array) -> None:
        ...
        for idx, face in enumerate(self.simplicial_feature_set.face_set):
            ...
            if len(face) == 5:
                ...
                face_color = SimplicialElements.triangle_colors[idx % len(SimplicialElements.triangle_colors)]
        ...
        for (x, y) in tetrahedrons:
            plt.fill(x,
                     y,
                     color=SimplicialElements.tetrahedron_color,
                     alpha=0.6,
                     edgecolor='grey',
                     hatch='////')
