...
class GraphToSimplicial(object):
    ...
    def __init__(self,
                 ...
                 simplice_types: SimpliceTypes) -> None:
        """
        ...
        @param simplice_types: Type of Simplices to be collected from the graph
        @type simplice_types: Enumerator
        """
        self.data = data
        # The maximum rank of the simplices in the graph is computed from the type of simplicial to generate
        self.max_rank = simplice_types.value
        self.dataset_name = dataset_name
        self.threshold_clique_complex = threshold_clique_complex

    def __call__(self, *args, **kwargs) -> tnx.SimplicialComplex:
        """
        Method to generate a simplicial complex from a Graph
        @param args: List of arguments
        @param kwargs: Dictionary of arguments
        @return: toponetx Simplicial Complext
    ...
    @staticmethod
    def count_simplices_by_type(simplicial_complex: tnx.SimplicialComplex) -> Dict[AnyStr, int]:
        """
        ...
        sorted_simplices = sorted(simplicial_complex.simplices, key=len)
        return {GraphToSimplicial.types_map[length]: sum(1 for _ in group)
                for length, group in groupby(sorted_simplices, key=len) if length < 5}
